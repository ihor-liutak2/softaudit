...
export class ValidationService {
  ...
  private get projectsCollection(): CollectionReference<DocumentData> {
    return collection(this.firestore, 'validationProjects');
  }

  private get itemsCollection(): CollectionReference<DocumentData> {
  ...
  async saveProject(project: ValidationProject): Promise<void> {
    const id = project.id;
    const ref = doc(this.projectsCollection, id);
    const data: ValidationProject = { ...project, updatedAt: new Date().toISOString() };
  ...
  async saveItem(item: ValidationItem): Promise<void> {
    ...
    const data: ValidationItem = { ...item, updatedAt: new Date().toISOString() };
    await setDoc(ref, data, { merge: true });
